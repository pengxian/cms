class ImPost < ApplicationRecord
  self.table_name = 'im_posts'
  include AASM

  belongs_to :user, class_name: 'AdminUser'
  has_many :attachments, class_name: 'ImAttachment' , dependent: :destroy
  ...
  def enqueue
    record = Moment::Service::Post.new
  ...
  # 将表单传来的数据转化为字符串
  def gen_tag_ids
    self.tag_ids = JSON.parse(self.tag_ids).reject(&:empty?).join(',')
  end
